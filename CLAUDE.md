...
- **README 에 쓴 수치는 낡는다** — 상수나 실측치를 인용했으면 코드와 대조하고, 가능하면 테스트로 잠근다(수치를 문서에만 두지 말 것). 실측 하루에 4개가 어긋나 있었다.
- **판정 기준은 회귀를 일부러 만들어 확인한다** — 통과만으로는 그 기준이 무언가를 지킨다는 증거가 안 된다. 기준을 최악의 회귀값 '위'에 세우면 영영 안 걸린다(감쇠비를 1.0 에 뒀는데 회귀가 정확히 1.00 이었다).
